<template>
    <div class="numpad">
        ...
        <div class="numpad-values" @click="onDisplayValueClick">
            <span :id="displayValueDomId" class="numpad-value" :class="currentDisplayNumClass">{{ currentDisplay }}</span>
        </div>

        <f7-popover class="paste-context-menu-popover" :target-el="'#' + displayValueDomId"
        ...
        <div class="numpad-buttons">
            ...
            <!-- Zero-fraction currencies (VND and 15 others) get repeated-zero keys instead of a
                 decimal separator. Four keys share the row, so they are narrower than the 3-key rows. -->
            <f7-button class="numpad-button numpad-button-num numpad-button-num-narrow" @pointerup="inputRepeatedNum(0, 3)"
                    v-if="!supportDecimalSeparator">
                <span class="numpad-button-text numpad-button-text-normal">{{ `${digits[0]}${digits[0]}${digits[0]}` }}</span>
            </f7-button>
            <f7-button class="numpad-button numpad-button-num numpad-button-num-narrow" @pointerup="inputRepeatedNum(0, 2)"
                    v-if="!supportDecimalSeparator">
                <span class="numpad-button-text numpad-button-text-normal">{{ `${digits[0]}${digits[0]}` }}</span>
            </f7-button>
            <f7-button class="numpad-button numpad-button-num" :class="{ 'numpad-button-num-narrow': !supportDecimalSeparator }" @pointerup="inputNum(0)">
                <span class="numpad-button-text numpad-button-text-normal">{{ digits[0] }}</span>
            </f7-button>
            <f7-button class="numpad-button numpad-button-num" :class="{ 'numpad-button-num-narrow': !supportDecimalSeparator }" @pointerup="backspace" @taphold="clear()">
...
// Unique per instance: more than one pad can exist on a page (sheet plus inline quick-add),
// and the paste popover targets this element by id.
const displayValueDomId = `numpad-value-${useId()}`;
...
// Repeated-zero keys route through inputNum so they inherit its clamping; a press that would
// exceed maxValue simply stops early rather than being rejected outright.
function inputRepeatedNum(num: number, times: number): void {
...
// Re-seed the displayed value from modelValue. The sheet calls this when it opens so the pad
// always starts from the current amount rather than whatever was last typed.
...
/* Four keys share the bottom row for zero-fraction currencies (000, 00, 0, backspace),
   so they split the same 80% four ways instead of three. */
.numpad-button-num-narrow {
    width: 20%;
}
